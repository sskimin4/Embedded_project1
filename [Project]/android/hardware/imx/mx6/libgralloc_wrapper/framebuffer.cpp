...
static volatile int hyun=0;
static volatile int doing=0;
static char * fix_ptr;
static volatile int final_array[MAX_ROW][MAX_COL];
static volatile int arr_prev[MAX_ROW][MAX_COL];
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer);

struct framebuffer_device_t* dev_hyun;
buffer_handle_t buffer_hyun;
int fix_x;
int fix_y;

static void fb_puzzle(char* fb_ptr, private_module_t* m)
{
    static int h_flag=0;
    static int called_cnt = 0;
    static int i_flag = 0;
    //static int final_array[MAX_ROW][MAX_COL];
    //static int arr_prev[MAX_ROW][MAX_COL];
    ...
    if (m->info.bits_per_pixel == 32)
      {
        ...
        if (puzzle_fd < 0)
          {
            ALOGE("[@@YIH@@] <%s,%d> #%d# opening failed (/dev/puzzle_dev)!\n", __FUNCTION__, __LINE__, called_cnt);
            return;
          }
            ALOGE("[@@YIH@@] <%s,%d> #%d# opening success!\n", __FUNCTION__, __LINE__, called_cnt);

        struct __puzzle_data puzzle_data;
        int puzzle_enable = 0;

        ioctl(puzzle_fd, PUZZLE_GET_ENABLE, &puzzle_enable);

        if (puzzle_enable == 0)
          {
            ALOGE("[@@YIH@@] <%s,%d> #%d# PUZZLE DISABLED!\n", __FUNCTION__, __LINE__, called_cnt);
            close(puzzle_fd);
            return;
          }
            ALOGE("[@@YIH@@] <%s,%d> #%d# PUZZLE ENABLED!\n", __FUNCTION__, __LINE__, called_cnt);

        ioctl(puzzle_fd, PUZZLE_GET, &puzzle_data);


        int row_cnt = puzzle_data.row;
        int col_cnt = puzzle_data.col;

        if (row_cnt <= 0 || row_cnt > MAX_ROW ||
            col_cnt <= 0 || col_cnt > MAX_COL||(row_cnt==2&&col_cnt==2))
          {
            ALOGE("[@@YIH@@] <%s,%d> #%d# PUZZLE DATA STRANGE!!!! row_cnt = %d, col_cnt = %d\n", __FUNCTION__, __LINE__, called_cnt, row_cnt, col_cnt);
            close(puzzle_fd);
            return;
          }
            ALOGE("[@@YIH@@] <%s,%d> #%d# PUZZLE DATA MAYBE GOOD?!\n", __FUNCTION__, __LINE__, called_cnt);
        
        int (*arr)[MAX_COL] = puzzle_data.mat;
        int puzzle_change=0;

        ioctl(puzzle_fd,PUZZLE_CHANGE_ENABLE,&puzzle_change);
        //close(puzzle_fd);
        int prev_r=-1,prev_c=-1;
        int curr_r=-1,curr_c=-1;
        if(puzzle_change){
            struct __puzzle_data puzzle_data2;
            h_flag=0;
            //ioctl(puzzle_fd,PUZZLE_GET,&puzzle_data2);
        ...
        */
        ...
        for (int r = 0; r < row_cnt; ++r)
          {
            for (int c = 0; c < col_cnt; ++c)
              {
                if (arr[r][c] == r * col_cnt + c)
                  continue;
                int tr = arr[r][c] / col_cnt;
                int tc = arr[r][c] % col_cnt;
                for (int y=0; y<tmp_height/row_cnt; ++y)
                  {
                    int* ptr = (int*)(fb_ptr + (y + yoffset + r * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + c * (tmp_width/col_cnt)) * bpp);
                    int* tptr = (int*)(fb_ptr + (y + yoffset + tr * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + tc * (tmp_width/col_cnt)) * bpp);
                    for (int x=0; x<tmp_width/col_cnt; ++x, ++ptr, ++tptr)
                      {
                        int tmp = *ptr;
                        *ptr = *tptr;
                        *tptr = tmp;
        ...
        if(i_flag==0){
            hyun=1;
            i_flag=1;
            int arr_id[MAX_ROW][MAX_COL];
            
            for(int i=0;i<row_cnt;i++){
                for(int j=0;j<col_cnt;j++){
                    arr_id[i][j]=i*col_cnt+j;
                }
            }
            for (int r = 0; r < row_cnt; ++r)
            {
                for (int c = 0; c < col_cnt; ++c)
                {
                    if (arr[r][c] == arr_id[r][c])
                        continue;
                    int rr,cc;
                    for(int i=0;i<row_cnt;i++)
                        for(int j=0;j<col_cnt;j++){
                            if(arr_id[i][j]==arr[r][c]){
                                rr=i;
                                cc=j;
                                break;
                            }
                ...
                for (int y=0; y<tmp_height/row_cnt; ++y)
                  {
                    int* ptr = (int*)(fb_ptr + (y + yoffset + rr * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + cc * (tmp_width/col_cnt)) * bpp);
                    int* tptr = (int*)(fb_ptr + (y + yoffset + r * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + c * (tmp_width/col_cnt)) * bpp);
                    for (int x=0; x<tmp_width/col_cnt; ++x, ++ptr, ++tptr)
                      {
                        int tmp = *ptr;
                        *ptr = *tptr;
                ...
                int tmp = arr_id[rr][cc];
                arr_id[rr][cc] = arr_id[r][c];
                arr_id[r][c] = tmp;
                }
            }
        }
        ...
        if(puzzle_change){
            for (int y=0; y<tmp_height/row_cnt; ++y)
                  {
                    int* ptr = (int*)(fb_ptr + (y + yoffset + prev_r * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + prev_c * (tmp_width/col_cnt)) * bpp);
                    int* tptr = (int*)(fb_ptr + (y + yoffset + curr_r * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + curr_c * (tmp_width/col_cnt)) * bpp);
                    for (int x=0; x<tmp_width/col_cnt; ++x, ++ptr, ++tptr)
                      {
                        int tmp = *ptr;
                        *ptr = *tptr;
            ...
            for (int c = 0; c < col_cnt; ++c)
              {
                for (int y=0; y<tmp_height/row_cnt; ++y)
                  {
                    int* ptr = (int*)(fb_ptr + (y + yoffset + r * (tmp_height/row_cnt)) * m->finfo.line_length + (xoffset + c * (tmp_width/col_cnt)) * bpp);
                    if (y < border || y >= tmp_height/row_cnt - border)
                      {
                        for (int x=0; x<tmp_width/col_cnt; ++x, ++ptr)
                          {
                            ((unsigned char*)ptr)[0] = 0x05;
                            ((unsigned char*)ptr)[1] = 0x00;
                            ((unsigned char*)ptr)[2] = 0x99;
                            ((unsigned char*)ptr)[3] = 0;
                          }
                      }
                    else
                      {
                        for (int x=0; x<border; ++x, ++ptr)
                          {
                            ((unsigned char*)ptr)[0] = 0x05;
                            ((unsigned char*)ptr)[1] = 0x00;
                            ((unsigned char*)ptr)[2] = 0x99;
                        ...
                        ptr += (tmp_width/col_cnt - border - 1);
...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        m->info.activate = FB_ACTIVATE_NOW;     //VBL
        m->info.yoffset = offset / m->finfo.line_length;

        ALOGE("[@@YIH@@] <%s,%d> hnd->base = %p, m->framebuffer->base = %p, vaddr = %p, offset = %u\n", __FUNCTION__, __LINE__, hnd->base, m->framebuffer->base, vaddr, offset);

        fb_puzzle((char*)m->framebuffer->base, m);
